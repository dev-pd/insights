"""Extraction prompt v1.7 — LLM-driven noise detection via is_noise schema field.

Round 7 closes the absurd-historic-frame loop properly: instead of trying
to coerce the model into producing a "correct" sentiment/themes/actions
on nonsense input, give the model an explicit channel to say "this is
noise, skip it" via the new `is_noise: bool` schema field.

The full pipeline:
1. LLM sees absurd input + this prompt's noise-detection rule.
2. LLM returns is_noise=true (other fields populated as best guess).
3. Worker sees is_noise=true → row goes to status='skipped' with
   skip_reason='noise' (mirrors the language!='en' skip pattern).
4. Dashboard shows it as a Skipped row, not as legitimate negative feedback.
   Stats / KPIs / theme counts ignore it.

Replaces the v1.6 attempt to distill absurd inputs into "improve product
quality" — which technically passed evals but quietly added nonsense to
the dashboard's negative count and theme tallies.

All v1.6 guidance carries over verbatim except:
- New "Noise detection" section in the prompt body.
- The absurd-framing rule for action items is removed (no longer needed —
  noise inputs are skipped before action items matter).
...
**Context:** This product launched in 2020. Treat references to events significantly before that ("1000 years ago", "centuries ago", "back in the 1800s", "ancient times", "since the dawn of time") as absurd or hyperbolic literary devices, not literal claims.

**Noise detection:** Some inputs are clearly nonsensical and contain no real underlying product signal — impossible timeframes given the 2020 launch ("worked 1000 yrs ago", "centuries ago"), pure fiction with no actual product complaint ("the dragons in the app are too aggressive"), or gibberish-with-real-words ("the green spoon morning under timeline"). For these, set `is_noise: true` — the worker will skip the row. The other fields (sentiment, themes, action_items, language) must still be populated with your best guess but will not be used downstream. Default to `is_noise: false` — only flag obvious noise. Real complaints with poetic, hyperbolic, or sarcastic framing are NOT noise (e.g., "this product is the worst thing since unsliced bread" is real negative feedback).
...
- is_noise: boolean flag for nonsensical input (see Noise detection above)
...
- Return an empty list when the feedback is pure praise with NO specific ask, pure observation, or has no specific change the company should make.
"""

VERSION = "extraction/v1.7"
